...
import wave, struct, numpy
	# deprecated from 7.11: replaced by _Chunk class in wave
...
class waveread(wave.Wave_read):
    def initfp(self, file):
        ...
        # IMPORTANT: use wave._Chunk, not chunk.Chunk
        self._file = Chunk(file, bigendian=False)

        if self._file.getname() != b'RIFF':
            raise wave.Error('%s does not start with RIFF id' % wavname)

        if self._file.read(4) != b'WAVE':
            raise wave.Error('%s is not a WAVE file' % wavname)
        ...
        while True:
            ...
            if chunkname == b'fmt ':
                ...
                for _ in range(numcue):
                    (_, _, _, _, _, sampleoffset) = struct.unpack(
                        '<II4sIII', chunk.read(24)
                    )
                    if sampleoffset > self._cue:
                        self._cue = sampleoffset

            elif chunkname == b'smpl':
                header = struct.unpack('<9I', chunk.read(36))
                numsampleloops = header[7]
                if numsampleloops > 0:
                    (_, _, start, end, _, _) = struct.unpack('<6I', chunk.read(24))
